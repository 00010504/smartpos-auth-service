...
import (
	...
	"github.com/Invan2/invan_auth_service/events/handlers"
...
func NewPubSubServer(log logger.Logger, producer *kafka.Producer, consumer *kafka.Consumer, strgPG storage.StoragePG, strgR storage.StorageR) (PubSubServer, error) {
	return &pubSubServer{
		log:           log,
		producer:      producer,
...
func (p *pubSubServer) registerConsumers() {
	handlerV1 := handlers.NewHandler(p.log, p.strgPG, p.strgR)
